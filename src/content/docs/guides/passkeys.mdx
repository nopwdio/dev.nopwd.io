...
description: A guide to integrate Passkeys authentication.
---
...
To create a passkey, just add the `<np-passkey-create>` compontent and set the `token` attribute with the user access token.
...
<script>
  const passkeyRegisterButton = document.querySelector("np-email-login");
