...
To create a passkey, just add the `<np-passkey-register>` compontent and set the `token` attribute from the authenticated user.
...
## (2/2) Authenticating with Passkeys
...
:::tip[What if the user didn't create a passkey?]
In that situation, this component will simply act as a classic email input element for users without a Passkey.
You can use it as an email input to pass the email value to the `<np-email-login>` component to fallback to the email link authentication flow.
:::
