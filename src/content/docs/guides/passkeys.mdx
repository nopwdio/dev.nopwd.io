...
The `np:auth` event emitted from the email authentication [`<np-email-auth>`](/reference/components/np-email-auth) compontent contains a `suggest_passkeys` attribute.
...
:::tip[importing component]
You can import it with CDN or NPM. [Learn more](/reference/components/#how-to-install-it).
:::
...
<script type="module">
  ...
  el.addEventListner("np:session", (e) => {
