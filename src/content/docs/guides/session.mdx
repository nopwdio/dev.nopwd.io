...
There are two settings for doing so and you can specify the authentication components attributes:
 - `sessionlifetime`: to set the maximum session lifetime (up to a year);
 - `sessionidlelifetime` : to optionaly set the inactivity timeout.

```html
<!-- by default, session lifetime is set to 24 hours without any inactivity threshold -->
<np-email-auth></np-email-auth>

<!-- to set 7 days session lifetime -->
<np-email-auth sessionlifetime="604800"></np-email-auth>

<!-- to set one day session lifetime with 1hour inactivity timeout -->
<np-email-auth sessionlifetime="604800" sessionidlelifetime="86400"></np-email-auth>
```
...
```ts
import { get } from "@nopwdio/sdk-js/core/session";
...
const JWKS = createRemoteJWKSet(new URL("https://api.nopwd.io/v1/jwks"));
...
```ts
import { revoke } from "@nopwdio/sdk-js/core/session";
