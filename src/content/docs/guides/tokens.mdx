...
Authentication components (such as [`np-email-login`](/reference/components/np-email-login) [`np-passkey-login`](/reference/components/np-passkey-login)) emit an `np:auth` event. This event contains the access `token` representing the user authentication proof.
