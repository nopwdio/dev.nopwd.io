...
Authentication components (such as [`np-email-auth`](/reference/components/np-email-auth) [`np-passkey-conditional`](/reference/components/np-passkey-conditional)) emit an `np:auth` event. This event contains the access `token` representing the user authentication proof.
...
const { payload } = await jwtVerify(token, JWKS, {
  issuer: "nopwd.io",
  audience: "nopwd.rocks", // your own domain
});
```
